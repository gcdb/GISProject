...
to help us. Read the code chunks below into your RStudio environment. Enter the lines in blue into your\
console.
...
Now that we have more complete csv file, found.csv, we can add it in our QGIS as a point layer\
1. Under the Layer menu bar, select Add Layer -\> Add Delimited Text Layer\
...
5. Save Selected Features As all_primary_schools into your Project GeoPackage 6. Open the attribute table of all_primary_schools\
7. Under the school_name field, look for MACPHERSON PRIMARY SCHOOL\
8. Enable the Toggle Editing tool and delete away one of the duplicated row
...
Primary School, Loyang Primary School, and Macpherson Primary School) that have already been added\
in. This can be done quicker by filtering the school_name field.\
...
From the attribute table of all_primary_schools, we can click on the Invert Feature Selection button that\
would select the other 183 primary schools.\
...
![](images/paste-C6A9A283.png) 8. A temporary layer, Grid, appears under Layers panel\
...
11. Remove temporary layer, Grid from Layers panel 12. Under the Vector menu bar, select Geometry Tools -\> Centroids
...
a\. Buffer The first method we will use to find the affected residentials of the closure of schools will be by using\
...
b. QNEAT3\
i. Shortest Path\
The second method we will use to find the affected residentials of the closure of schools will be by using\
the shortest path method 1. Under Processing Toolbox, search for QNEAT3\
2. Select the Iso-Area as Polygons (from Layer)

![](images/paste-90C45CBB.png)

Since we are trying to find the shortest path of 1km and 2km respectively, for two modes of transport --\
by walking and by private transport, we will be creating a total of 4 pairs of output polygon and\
...
ii. Average out SP & FT\
    After we have created all 8 different layers via the shortest path and fastest time with QNEAT3, we can\
    calculate the number of affected residentials in each layer.\
...
![](images/paste-FD77F515.png) 3. After each temporary Service area (lines) layer is added to your Layers panel, search for convex\
under Processing Toolbox and select Convex hull

![](images/paste-2181CDAA.png) 4. Select Service (area) lines as your Input layer and click Run\
...
primary school count, one (consisting of all and curr) for each buffer
...
Under the Vector menu bar, select Analysis Tools -\> Count Points in Polygon. You will create two pairs of\
primary school count, one (consisting of all and curr) for each walking polygon ![](images/paste-D49896E7.png)
...
Under the Vector menu bar, select Analysis Tools -\> Count Points in Polygon. You will create two pairs of\
primary school count, one (consisting of all and curr) for each driving polygon ![](images/paste-94A4B216.png)
